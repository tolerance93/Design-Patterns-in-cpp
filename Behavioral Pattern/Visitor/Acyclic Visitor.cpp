...
// acyclic visitor: based on RTTI(Run Time Type Information)
...
// 객체 자체가 직접 visit() 멤버 함수를 가질 경우 호출할 모든 타입마다 visit()멤버 함수를 정의하여 오버로딩할 수 있어야 한다.
// 그렇게 하면 순환 종속성이 발생한다 <= visit()이 여기 없는 이유
struct Expression
{
